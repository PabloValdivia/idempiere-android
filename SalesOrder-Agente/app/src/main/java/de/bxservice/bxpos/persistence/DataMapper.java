...
import de.bxservice.bxpos.persistence.helper.PosBPartnerHelper;
...
public class DataMapper implements Serializable {
    ...
    public boolean save(Object object) {
        ...
        else if(object instanceof CBPartner)
            success = createBPartner((CBPartner) object);
    ...
    public boolean update(Object object) {
        ...
        if(object instanceof PosUser)
            ...
            success = updateBPartner((CBPartner) object);
        else if(object instanceof POSOutputDevice)
    ...
    private boolean createBPartner(CBPartner bpartner) {

        PosBPartnerHelper bpartnerHelper = new PosBPartnerHelper(mContext);

        if (bpartnerHelper.createBPartner(bpartner) == -1) {
            Log.e(LOG_TAG, "Cannot create bpartner " + bpartner.getBPartnerName());
            return false;
        }
        Log.i(LOG_TAG, bpartner.getBPartnerName() + " created");
        return true;
    }

    private boolean updateBPartner(CBPartner data) {
        PosBPartnerHelper bpartnerHelper = new PosBPartnerHelper(mContext);

        if (bpartnerHelper.updateBPartner(data) != -1) {
            Log.i(LOG_TAG, "BPartner updated");
        }
        else {
            Log.e(LOG_TAG, "Cannot update bpartner");
            return false;
        }
    ...
    public CBPartner getBpartner(long id) {
        PosBPartnerHelper bpartnerHelper = new PosBPartnerHelper(mContext);
        return bpartnerHelper.getBPartner(id);
    }
    ...
    public List<CBPartner> getBPartners() {
        PosBPartnerHelper bpartnerHelper = new PosBPartnerHelper(mContext);
